...
def iniciar_cli():
    ...
    while True:
        ...
        if opcion == "1":
            ...
            try:
                id_ingresado = int(input("Ingrese el ID de la canción: "))
            ...
            try:
                id_ingresado = int(input("Ingrese el ID de la canción base: "))
                cancion = biblioteca.buscar_por_id(id_ingresado)
                if cancion:
                    ids_derivados = biblioteca.obtener_versiones_derivadas(id_ingresado)
                    print(f"\n--- Versiones derivadas de '{cancion.titulo}' (ID {id_ingresado}) ---")
                    if ids_derivados:
